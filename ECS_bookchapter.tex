...
\gen{Agree with Geir's suggestion to remove MEG from the figure.}
\tvnnote{Done!}
}
...
\snnote{Liker veldig godt oppklarende setning 2 her! Kan ogs\aa{} skrive om setning 1 s\aa nn her:}

In neural tissue, \sntxt{there will be no current monopole contribution to the extracellular potential, }
%In neural tissue, current monopoles are unphysical, 
...
\gen{I think we should have a short subsection here on the modeling of electrodes, that is (i) ideal point electrodes (as assumed in all present examples), (ii) the disc-electrode approximation, and (iii) FEM modeling.}
\tvntxt{
The simplest and most commonly used approach to modeling extracellular recording electrodes is to calculate the extracellular potential at a specific point following one of the approached outlined above, and use this as the recorded potential. Implicitly, this 
assumes that the electrode itself does not affect the extracellular potential (ideal electrode), and that the extracellular potential does not vary substantially over the surface of the electrode (this assumption was used for all simulation examples in this chapter).
A straight-forward extension is the disc-electrode approximation where the potential is evaluated at a number of points on the (virtual) electrode surface, and the average calculated. This approach takes into account the physical extent of the electrode, but not any effect the electrode itself might have on the electric potential \citep{Ness2015}. A third, and much more comprehensive approach is to use the Finite Element Method to model the electrode \citep{Moulin2008, Ness2015}, or the electrode shank \citep{Moffitt2005, Buccino2019b}. 

Note that for modelling current stimulation (as opposed to recording), more complex electrode models might be needed due to electrode polarization effects \citep{McIntyre2001, Martinsen2008, Joucla2012}.
}
...
\tvntxt{All filters where 4th order Butterworth filters in forward-backward mode \citep{NeuroEnsamble2017}. A relatively low cut-off frequency of 50~Hz was chosen for the LFP low-pass filter for illustrative purposes. The MUA was first high-pass filtered above 300 Hz (E and F), then rectified and low-pass filtered below 300 Hz (F).}
\gen{Vi boer si presist hvordan filtreringen er gjort.}
\tvnnote{OK?}
}
\label{fig:LFP_MUA}
\end{figure}
...
\snnote{S\aa{} at kommentaren som l\aa{} her tidligere var fjernet, men jeg er egentlig litt enig. Blir det bedre s\aa nn her?}
