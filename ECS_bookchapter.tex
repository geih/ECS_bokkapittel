...
\label{sec:eldiff}
\snnote{After discussing with Geir and Torbjorn, we decided to change the notation a bit, such that lowercase $j_k$ is flux density, lowercase i is current density and uppercase I is current. Torbjorn and Geir: can you please double check that I got it right?}
\snnote{Do we write equation as Eq. or eq.? Now we use both..}
...
\label{sec:VC_theory}
In simulations of morphologically complex neurons, we typically compute a set of transmembrane current sources for each neuronal segment \citep{Koch1999}. Commonly, one assumes that the extracellular potential does not  \sntxt{affect} the neurons (i.e., no ephaptic coupling), since extracellular potentials are typically much smaller than the membrane potential of $\sim$-70~mV. 
...
\snnote{How about exchanging the following "in ${\bf r} = 0$" with "at the origin"? For me, this would make the next ${r}$ easier to understand, but I guess this is a matter of taste..}
If we consider the simplest possible case of a single point current source $I_1$ in ${\bf r}=0$ 
in an isotropic medium, the current density ${\bf i} = -\sigma {\bf \nabla} \phi$ through a spherical shell with area $4\pi r^2$ must equal $I_1/4\pi r^2~{\bf \hat{r}}$. 
Due to the spherical symmetry of the problem, ${\bf \nabla} \phi = \partial \phi/ \partial r~{\bf \hat{r}}$, and integration with respect to $r$ gives us:
...
When estimating the extracellular potential at large distances, it can often be useful to express Eq.~\eqref{eq:VCtheory} in terms of an infinite series.
Specifically, the electric potential outside a volume containing a combination of current sinks and sources, can be precisely described by the multipole expansion \citep{Nunez2006}:

\begin{equation}\label{eq:multipole}
\phi(R) = \frac{C_{monopole}}{R} + \frac{C_{dipole}}{R^2} + \frac{C_{quadrupole}}{R^3} + \frac{C_{octupole}}{R^4} + ... .
\end{equation}
This equation holds when when the distance $R$ from the center of the volume to the measurement point is larger than the distance from volume center to the most peripheral source \citep{Jackson1998}. Here, the monopole contribution $C_{monopole}$, dipole contribution $C_{dipole}$ and quadrupole contribution $C_{quadrupole}$, etc are \snnote{somehow} defined in \cite{Riera2012}.
...
\snnote{Top row, panel B: "two currentS". Top row, panel C: "distributed return currentS". Bottom row, panel A: $\phi$-font different from notation below color bar.}
}
\label{fig:EP_morph}
\end{figure}
...
The current dipole approximation is however not an obvious candidate for computing ECoG signals, as the ECOG electrodes are located too close to the signal sources, see \cite{Hagen2018}.
...
\snnote{Torbjorn: Was it something in the means of the last sentence above, that you had in mind? Do you have any suggestions for papers to cite? Feel free to make changes!}

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.6\textwidth]{head_models.pdf}
\end{center}
\caption{\textbf{The four-sphere head model and the NY Head model}. EEG signals from population dipole resulting from neural activity in cortical microcircuit model of $1$mm of primary visual cortex, see \cite{Hagen2016} Fig 1.
