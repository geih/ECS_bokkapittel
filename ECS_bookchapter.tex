...
\snnote{It would be nice with a clearer distinction between current and current density..}
...
\snnote{current dipole approximation, suggestion 1:}

\sntxt{
Due to current conservation in neural tissue, a current source $I_1 = I$ will always be accompanied by a respective current sink $I_2 = -I$. Here we envision a two-compartment neuron model, like the one in Fig.~\ref{fig:EP_morph}, panel B.
We can find the exact electric potential from these two currents, by applying eq. \eqref{eq:VCtheory}:
\begin{equation}\label{eq:PHItwomonopoles}
\Phi(\mathbf{r}) = \frac{I}{4 \pi \sigma} \left( \frac{1}{|\mathbf{r} - \mathbf{r}_1|} - \frac{1}{|\mathbf{r} - \mathbf{r}_2|} \right),
\end{equation}
where $\mathbf{r}_1$ and $\mathbf{r}_2$ are the locations of current sink $I_1$ and current source $I_2$. 

This expression can be generalized by introducing the current dipole moment $\mathbf{p}$:

\begin{equation}\label{eq:dip}
\mathbf{p} = \sum_{n=1}^N I_n \mathbf{r}_n, 
\end{equation}
where $I_n$ is the transmembrane current at location $\mathbf{r}_n$.



For our two-compartment model, the current dipole moment can be written as

\begin{equation}\label{eq:DIPtwomonopoles}
\mathbf{p} = I_1 \mathbf{r}_1 + I_2 \mathbf{r}_2 = I (\mathbf{r}_1- \mathbf{r}_2) = I \mathbf{d},
\end{equation}
where $\mathbf{d} = \mathbf{r}_1 - \mathbf{r}_2$ gives the dipole length ($d$) and orientation of the current dipole moment.


Applying the cosine rule and a binomial expansion we find that $\frac{1}{|\mathbf{r} - \mathbf{r}_1|} - \frac{1}{|\mathbf{r} - \mathbf{r}_2|} \approx \frac{d \cos \theta}{R^2}$, for $R >> d$, where $R$ is the distance between the midpoint of the dipole and $\mathbf{r}$.
By inserting this relation into eq.~\eqref{eq:PHItwomonopoles}, we arrive at the so-called current dipole approximation:

\begin{equation}\label{eq:CDA}
\Phi(\mathbf{R}) \approx \frac{1}{4 \pi \sigma} \frac{|\mathbf{p}| \cos \theta}{R^2},
\end{equation}
which gives a reasonable approximation of the electric potential measured at a distance $R$ larger than $3d$ or $4d$ away from the dipole $\mathbf{p} = I \mathbf{d}$ \cite{Nunez2006}. 

In order to get a better understanding of what we lose by using the current dipole approximation over the point source approximation, we can take a look at the multipole expansion for electric current point sources.

Imagine a volume containing a distribution of current sinks and sources $I_k$ positioned at $\mathbf{r}_k$, as illustrated in Fig. x, where $\mathbf{r}_c$ marks the center of the volume, and $R = |\mathbf{r}_c - \mathbf{r}|$ is the distance from the center of the current distribution to the point where we want to calculate the potential.

Making use of the cosine rule and a binomial expansion for $1/|\mathbf{r} - \mathbf{r_k}|$, we can rewrite eq.~\eqref{eq:VCtheory}. The result is the multipole expansion for current sources:
...
valid for $R > r_k^c$, where $r_k^c$ is the distance from the current distribution center $\mathbf{r}_c$ to current source $I_k$ at $\mathbf{r}_k$, and $\theta_k$ is the angle between $\mathbf{r}_c^k$ and $\mathbf{R}$. Here, the various terms are often referred to as the monopole contribution, the dipole contribution, the quadrupole contribution, etc..
For a current dipole, consisting of a single current sink and a single current source, the monopole term vanishes. The dipole term gives us eq. \eqref{eq:CDA}. Perhaps more surprisingly, the quadrupole and other higher-order terms will also give nonzero contributions to the electric potential from a current dipole! These higher-order terms will be negligible for $R > 3d$ or $R > 4d$, since they fall more rapidly with distance. 

In other words, by using the current dipole approximation for computing the electric potential from a current dipole, we lose the quadrupole, octupole and higher-order contributions. This is a good approximation when the measurement point is far away from the current dipole.
}
...
\ghtxt{GH: Wouldn't it be nice to introduce $\mathbf{p}$ already in eq. \ref{eq:multipole}. As far as I can tell, eq. \ref{eq:grillfest} implies that $|\mathbf{p}| = \sum_{k=1}^N I_k r_k$ (no vectors anymore) and isn't then $r_k$ here the same as $r_k^c$ in the eq. \ref{eq:multipole}?
}
\ghnote{Check!}
...
\snnote{Short: How to compute ECoGs and EEGs, refer to equations in theory.}
