...
\label{sec:theory}
Extracellular potentials are generated by electrical currents in the extracellular space. The currents are in turn mediated by movement of ions, and can in principle include several components:
%
\begin{enumerate}
...
In order to solve a set (i.e., one for each ion species present) of equations like eq.~\ref{eq:NP}, one needs an expression for the electrical potential $\phi$. There are two main approaches to this. The physically most detailed approach is to use the Poisson-Nernst-Planck (PNP) formalism \citep{Leonetti1998, Leonetti2004, Lu2007, Lopreore2008, Nanninga2008, Pods2013, Gardner2015}. Within this formalism, $\phi$ is determined from Poisson's equation from electrostatics, 
\begin{equation}
...
If we multiply eq.~\ref{eq:JNP} by $F\cdot z_k$ and sum over all ion species, we get an expression for the net electrical current density due to all particle fluxes,
...
A numerically straightforward extension is the disc-electrode approximation where the potential is evaluated at a number of points on the electrode surface, and the average calculated \citep{Nunez2006, Linden2014}. \gen{Does Nunez talk about this?} \tvnnote{Yes, eq. (5.30) in the book, however, strangely they actually use a volume integral, so maybe just remove the reference?}
