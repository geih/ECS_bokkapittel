...
\item {\bf Point sources:} We represented the neuronal membrane currents as a single point source per neuronal segment. If we instead assumed that the currents from each segment $k$ were evenly distributed over the segment length ($\Delta s_k$), eq. \ref{eq:VCtheory} should be replaced with:
\begin{equation}
