...
Here the models have been developed and tested by comparison with membrane potentials recorded by intracellular electrodes in 
\emph{in vitro} settings. To pursue this mechanistic approach to network models in layered structures such as cortex or hippocampus, 
...
The standard way to compute extracellular potentials from neural activity is a two-step process:
\begin{enumerate}
\item Use networks of biophysically-detailed neuron models and compute the net transmembrane current in all neuronal segments in all neurons, and
\item use VC theory to compute extracellular potentials, ...
\end{enumerate}


\begin{itemize}
\item This book chapter describes: The origin of VC-theory, i.e., how it can be derived from the more detailed electrodiffusive theory 
...
\gen{I think this caption could be expanded a bit, also explaining why we include MEG signals even though it is not an electrical signal.}}
...
In neural tissue, current monopoles are unphysical, \gex{that is, $C_\text{monopole}=0$. This follows from the requirement inherent in the cable equation that the sum over all transmembrane currents, including the capacitive currents, across the neuronal surface has to be zero at all points in time.} 
\gen{Har vi en passende refereanse her?} 
\gex{Further,} 
