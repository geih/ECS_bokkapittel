...
Eq. \ref{eq:VCtheory} is often referred to as the point-source approximation \citep{Holt1999, Pettersen2008a}, since the membrane currents from a neuronal segment are assumed to enter the extracellular medium in a single point. An alternative scheme is the line-source approximation, where transmembrane currents from each segment is assumed to be evenly
distributed along the segment axis \citep{Holt1999, Pettersen2008a}.

The formula in eq. \ref{eq:VCtheory} (or the line-source version of it) represents the VC theory in its simplest form, and was based on a set of assumptions, some of which may be relaxed for problems where it is relevant: 
...
\item {\bf Linear extracellular medium:} Linear relationship ($\bf{I} = \sigma {\bf E}$) between the current density ${\bf I}$ and the electrical field, ${\bf E}$. This is essentially Ohm's law for volume conductors, and the relation is constitutive, meaning that it is observed in nature rather than derived from any physical principle \citep{Nunez2006, Pettersen2012}.
...
Volume conductor theory is the fundament for forward modeling of extracellular potentials both at the level of LFP (and MUA), ECoG or EEG. The theory is incorporated in the customized software LFPy \cite{Linden2014}, which in principle can be used to compute extracellular potentials on arbitrarily large spatial scales, surrounding arbitrarily large neuronal populations. In the following sections we shall review previous modeling works, and insights from simulating electrical potentials at these different scales.
...
\ghnote{Maybe not use current-conservation as a main topic, but rather talk about how extracellular potentials reflect morpholohy. Say something like: The extracellular potential arising from the activity of a single neuron is a reflection of not only its dynamical activity, but also its  morphology, and neurons with a large spatial extension generally give rise to larger extracellular potentials as there will be larger distances between current.}
...
The Local Field Potential is the Low-Frequency Part ($\lesssim$ 500~Hz) of the extracellular potentials, and it is among the oldest and most used brain signals in neuroscience \citep{Einevoll2013}. The LFP is expected to be dominated by large numbers of synaptic inputs to population of geometrically aligned neurons \citep{Nunez2006, Linden2011, Einevoll2013b}, and what determines a single neuron's contribution to the LFP is both the geometrical structure of the neuron, and the location of the synaptic inputs.  \ghn{Maybe move this synapse-stuff to the Population LFP-subsection. If we only look at a single neuron, like here, I guess spikes are more important.}
...
\label{sec:active}
\ghn{Perhaps make this stuff more compact, and not have it as separate subsection?}
