...
\label{sec:eldiff}
\snnote{After discussing with Geir and Torbjorn, we decided to change the notation a bit, such that lowercase $j_k$ is flux density, lowercase i is current density and uppercase I is current. Torbjorn and Geir: can you please double check that I got it right?} \tvnnote{Looks good to me}
\snnote{Do we write equation as eq. or Eq.? Now we use both..}
\tvnnote{I wrote Eq., but since most of theory is now eq. I changed all to this.}
...
By assuming that the tissue medium \tvntxt{can be approximated as} a volume conductor \citep{Holt1999, Linden2014}, one can then use the standard CSD equation (eq.~\ref{eq:CSDstandard}) to perform a forward modeling of the extracellular potential at each point in space surrounding the neuron(s). 
...
\end{equation}
where $r$ is the distance from the source \tvnnote{OK?}. If we have several point-current sources, $I_{1}, I_2, I_3, ... $, in locations ${\bf r_1}, {\bf r_2}, {\bf r_3} ... $, their contributions add up due to the linearity assumption (see sec.~\ref{sec:VC_assumptions}), and the potential in a point ${\bf r}$ is given by:
...
\subsubsection{\tvntxt{Current-dipole approximation}}
\tvnnote{Jeg skrev om litt her}
When estimating the extracellular potential \tvntxt{from a set of current sources} at large distances \tvntxt{relative to the distances between the current sources}, 
it can often be useful to express the current sorces in terms of \tvntxt{the current-dipole approximation. The extracellular potential}
%eq.~\eqref{eq:VCtheory} in terms of an infinite series.
%Specifically, the electric potential outside a volume containing a combination of current sinks and sources, 
can be precisely described by the multipole expansion \citep{Nunez2006},
...
The head models themselves introduce no essential frequency filtering of the EEG signal \citep{Pfurtscheller1975, Nunez2006, Ranta2017}, however, substantial spatial filtering will occur (Fig.~\ref{fig:foursphere_contour}). Additionally, the measured (or modelled) signals represent the average potential across the elecrode surface, and the large electrode sizes used in ECoG/EEG recordings can have important effect on the measured signals \citep{Nunez2006, Hagen2018, Dubey2019}.
