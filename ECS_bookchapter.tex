...
%In neural tissue, current conservation ensures that there is no current monopole contribution to the extracellular potential, that is, $C_\text{monopole}=0$.
\sntxt{Due to current conservation in neural tissue, there is no current monopole contribution to extracellular potentials, }\gex{that is, $C_\text{monopole}=0$.}   

%In neural tissue, there will be no current monopole contribution to the extracellular potential, due to current conservation, \gex{that is, $C_\text{monopole}=0$.}  
...
\snnote{Bedre naa?}
Further, the quadrupole, octupole and higher-order contributions decay rapidly with distance $R$. Consequently, the multipole expansion can be approximated by the dipole contribution for large distances, a simplification known as the current-dipole approximation \citep{Nunez2006}:
%%%
...
\end{equation}
%%%
In the case of a two-compartment neuron model (see Section~\ref{sec:ExtracellularPotentials}) with a current sink $-I$ at location $\mathbf{r}_1$ and a current source $I$ at location $\mathbf{r}_2$, the current dipole moment can be formulated as $\mathbf{p} = -I\mathbf{r}_1 + I\mathbf{r}_2 = I(\mathbf{r}_2 - \mathbf{r}_1) = I\mathbf{d}$, where $\mathbf{d}$ is the distance vector between the current sink and the current source, giving the dipole length $d$ and direction of the current dipole. The current-dipole approximation is applicable in the far-field limit, that is when $R$ is much larger than the dipole length. \sntxt{In general, this means when $R > 3d$ or $R > 4d$ \citep{Nunez2006}. Specifically, when computing the extracellular potential from a single apical synaptic input to a pyramidal cell, \cite {Linden2010} found that the measurement point must be about 1-2 mm away from the cell for the approximation to be applicable.}
...
\snnote{Jeg er litt usikker p\aa{} hva som er lurt \aa{} gj\o re her.. I Lind\'en2010 gjelder tommelfingerregelen for en spesiell pyramidalcelle med apikalt input. Jeg synes kanskje vi ogs\aa{} b\o r nevne Nunezs regel, siden denne er mer generell.}



%{Hva med forslag over? I Nunez snakker de generelt om n\aa r dipolapproksimasjonen kan predikere potensialet fra to monopoler, mens i Linden har vi et spesifikt eksempel der vi ikke bare ser p\aa  dipolapproksimasjonen i seg selv, men ogs\aa  den forenklinga vi gj\o r ved \aa  summere alle str\o mmer til \'e n enkelt dipol med \'e n enklet plassering.}
...
In contrast to pyramidal neurons, interneurons often lack any clear orientational specificity, meaning that the current dipoles \tvntxt{from individual interneurons, which can in themselves be sizable \citep{Linden2010},} do not align, leading to negligible \tvntxt{net} contribution to LFP signals, except indirectly through giving synaptic input to pyramidal cells \citep{Mazzoni2015,Telenczuk2016, Hagen2016}.
...
\subsection{MUA}
While LFPs are thought to mainly reflect the synaptic input to large populations of pyramidal neurons, the multi-unit activity (MUA) can be used to probe the population spiking activity \citep{Pettersen2008} (Fig.~\ref{fig:LFP_MUA} E,F). \tvntxt{In other words, the MUA holds complimentary information to the LFP. In particular, this} can be useful for some cell-types, like stellate cells and interneurons, which are expected to have very weak LFP contributions \citep{Linden2011}, but might still be measurable through their spiking activity. 
...
%\tvnnote{I liked both :-)}
we need to insert sharp electrodes into the brain. This highly invasive technique is quite common in animal studies, but can only be applied to humans when there is a clear medical need, for example in patients with intractable epilepsy \citep{Zangiabadi2019}. \sntxt{\sout{Fortunately} However}, electric potentials generated by neural activity extend beyond neural tissue \sntxt{and can also be measured outside of the brain:
Placing electrodes on the brain surface, electrocorticography (ECoG), is a technique that requires some surgery. With electroencephalogram (EEG), on the other hand, potentials are measured non-invasively, directly on top of the scalp.}
...
\gen{Her kan det hoeres ut som vi kategoriserer ECoG som non-invasive, og det er jo ikke riktig.}
\snnote{\AA! Det var ikke meninga. Ble dette bedre?}
...
\snnote{Har ikke f\aa tt skaffet meg denne enn\aa.=/ Geir, har du forslag?}
